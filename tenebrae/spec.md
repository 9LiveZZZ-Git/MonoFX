...
1. Author highlights a sentence → "Translate to…" → picks one of the **six** tongues the codex can write: Celan Basic, Celan High, Kerrackian, Kildaren, Calgridarian, Evernessian. *(Corrected 2026-08-22: this line read "seven tongues" and listed Rath-Speech. The Codex Omnilingua carries Rath-Speech as lore only — an id, a name, an epithet and a note, daggered "the Dead Tongue of the Wheel" — with no lexicon, no roots and no rules; it is excluded from the codex's own `ORDER`, so `buildLang` never runs over it and there is nothing to translate into. The tongue roster is read from the codex at runtime and is never hard-coded, so the app was always right and the spec was always wrong.)*
